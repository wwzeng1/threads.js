...
class FeedManager extends RESTManager {
    ...
    async fetchThreads(user, max_id) {
        let params = {
            user: String(user),
            max_id: max_id ? encodeURIComponent(max_id) : undefined
        };
        const queryString = Object.keys(params).filter(key => params[key] !== undefined).map(key => key + '=' + params[key]).join('&');
        return await this.request(`/api/v1/text_feed/profile/?` + queryString);
    }
    ...
    async fetchReplies(user, max_id) {
        ...
        const queryString = Object.keys(params).filter(key => params[key] !== undefined).map(key => key + '=' + params[key]).join('&');
        return await this.request(`/api/v1/text_feed/profile/replies/?` + queryString);
    }
